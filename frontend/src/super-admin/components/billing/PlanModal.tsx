...
export function PlanModal({
  ...
  const [monthlyPrice, setMonthlyPrice] = useState(
    plan?.monthlyPrice?.toString() || "49",
  );
  const [yearlyPrice, setYearlyPrice] = useState(
    plan?.yearlyPrice?.toString() || "490",
  );
  const [userLimit, setUserLimit] = useState(
    plan?.userLimit?.toString() || "10",
  );
  const [apiLimit, setApiLimit] = useState(plan?.apiLimit || "100,000 req/mo");
  const [storageLimit, setStorageLimit] = useState(
    plan?.storageLimit || "10 GB",
  );
  const [supportSla, setSupportSla] = useState(
    plan?.supportSla || "12h Support Response",
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSavePlan({
      ...
      userLimit:
        userLimit === "Unlimited" ? "Unlimited" : parseInt(userLimit) || 5,
      apiLimit,
      storageLimit,
      supportSla,
      features: [
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Configure pricing tier limits, features, and SLAs.
              </p>
            </div>
          </div>
          <button
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <Checkbox
                  ...
                  onCheckedChange={(c) => setCustomDomain(!!c)}
                />
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <Checkbox
                  ...
                  onCheckedChange={(c) => setAdvancedAnalytics(!!c)}
                />
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <Checkbox
                  ...
                  onCheckedChange={(c) => setPrioritySupport(!!c)}
                />
