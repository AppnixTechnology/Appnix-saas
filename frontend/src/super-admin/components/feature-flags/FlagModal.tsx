...
export function FlagModal({ isOpen, flag, onClose, onSaveFlag }: FlagModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSaveFlag({
      ...
      name: name.trim(),
      description: description.trim() || "Controls runtime platform feature availability.",
      isEnabled,
      environment,
      impactLevel,
      ...
      <div className="w-full max-w-md rounded-2xl border bg-card p-6 shadow-2xl animate-in">
        <div className="flex items-center justify-between border-b pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Toggle platform capabilities without redeployment.</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-4 text-xs">
          <div className="space-y-1">
            <label className="font-semibold text-muted-foreground">Feature Name *</label>
            <Input
              required
          ...
          <div className="space-y-1">
            <label className="font-semibold text-muted-foreground">Unique Flag Key *</label>
            <Input
              required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="font-semibold text-muted-foreground">Environment</label>
              <select
                value={environment}
                onChange={(e) => setEnvironment(e.target.value as FeatureFlag["environment"])}
            ...
            <div className="space-y-1">
              <label className="font-semibold text-muted-foreground">Impact Level</label>
              <select
                value={impactLevel}
                onChange={(e) => setImpactLevel(e.target.value as FeatureFlag["impactLevel"])}
                className="w-full h-9 rounded-md border border-input bg-background px-2.5 text-xs"
              >
