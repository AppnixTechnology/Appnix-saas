...
function DashboardCreateCampaignContent() {
    ...
    try {
      if (campaignId || wizardState.campaign.id) {
        ...
        if (newCampaignId) {
          ...
          router.replace(`/dashboard/campaigns/new?campaignId=${newCampaignId}`);
        } else {
...
export default function DashboardCreateCampaignPage() {
  return (
    <Suspense
      fallback={
        <div className="flex items-center justify-center min-h-[60vh]">
    ...
    >
      <DashboardCreateCampaignContent />
    </Suspense>
  );
}
